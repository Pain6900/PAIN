...
module.exports = {
 ...
 return message.reply("This channel dosen't support nsfw content")
   ...
   // {
    ...
    var url = await fetch("https://www.reddit.com/r/Hentai/random/.json");
    var random = await url.json();

    //send the Hi embed
        message.reply(new MessageEmbed().setColor("#FF00A6").setTitle(`Random Hentai | ${random[0].data.children[0].data.title}`)
